...
class CPU6502:
    ...
    def dump_memory(self, start: int, end: int, filename: Optional[str] = None):
    ...
    def print_registers(self):
        ...
        print(f"A=${self.a:02X} X=${self.x:02X} Y=${self.y:02X} SP=${self.sp:02X} PC=${self.pc:04X} [{flags}] Cycles={self.cycle_count}")
    
    def disassemble_opcode(self, pc: int) -> str:
        """Simple disassembler for debug output"""
        opcode = self.memory[pc]
        
        # Simple lookup for common opcodes - not complete but good for debugging
        disasm_table = {
            0xA9: "LDA #$%02X",
            0xAD: "LDA $%04X", 
            0xA5: "LDA $%02X",
    ...
    def run(self, start_pc: int, max_cycles: int = 1000000, trap_addresses: Optional[List[int]] = None):
        """Main execution loop"""
        self.pc = start_pc
        self.cycle_count = 0
...
@click.command()
@click.option('--load', 'load_specs', multiple=True, 
              help='Load file at address (format: filename@address_hex)')
@click.option('--start', type=str, default='0200',
              help='Starting PC address in hex (default: 0200)')
@click.option('--trap', 'trap_specs', multiple=True,
              help='Trap addresses in hex (execution stops when PC reaches these)')
@click.option('--trace', is_flag=True,
              help='Enable instruction trace output')
@click.option('--max-cycles', type=int, default=1000000,
              help='Maximum cycles to execute (default: 1000000)')
@click.option('--dump', type=str,
              help='Dump memory range after execution (format: start:end or start:end:filename)')
@click.option('--compare', type=str,
              help='Compare memory dump with file (format: start:end:filename)')
def main(load_specs, start, trap_specs, trace, max_cycles, dump, compare):
    ...
    for spec in load_specs:
        if '@' not in spec:
            click.echo(f"Error: Load spec must be in format 'filename@address'", err=True)
            sys.exit(1)
    ...
    if compare:
        ...
        if emulated == expected:
            ...
            for i, (e, a) in enumerate(zip(expected, emulated)):
                if e != a:
                    addr = cmp_start + i
                    click.echo(f"  First difference at ${addr:04X}: expected ${e:02X}, got ${a:02X}")
                    break
